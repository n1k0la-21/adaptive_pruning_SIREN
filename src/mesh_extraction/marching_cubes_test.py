...
def marching(scene: o3d.t.geometry.RaycastingScene, res: int, level=0.0, chunk_size=65536):
    """
    ...
    for start in range(0, num_voxels, chunk_size):
        ...
        # Evaluate SDF
        flat_points = voxel_corner_points.reshape(-1,3).cpu().numpy()
        tensor = o3d.core.Tensor(flat_points.astype(np.float32), dtype=o3d.core.Dtype.Float32)
        sdf_flat = scene.compute_signed_distance(tensor).numpy().reshape(-1)
        voxel_sdf = torch.from_numpy(sdf_flat).reshape(voxel_corner_points.shape[:2]).to(device)
